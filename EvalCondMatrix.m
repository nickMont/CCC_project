...
%% Solve min snap and calculate condition number
threshold = 0.2:0.2:3;
for i = 1:numel(threshold)
    tic;
    [SolCoeff,Cost,logCond(i)] = solveMinSnapInvTransformParameter(Waypoints,threshold(i));
    toc;
end

figure(5); plot(threshold,logCond); grid on;
xlabel('Threshold (s)'); ylabel('log_{10}Cond(M)');
